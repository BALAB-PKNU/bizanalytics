...
# 3주차: 학생 화면에서는 과제 명세에 공개된 형식만 확인한다.
_FORMAT_WEEK03 = {
    1: {"hit0": "set"},
    2: {"prevented0": "set"},
    3: {"n0": "int", "m0": "int", "pf0": "number"},
    4: {"threshold_tbl": ("table", ["pending > 0", "pending > 1", "pending > 4", "pending > 16"], ["replaced", "prevented"])},
    5: {"costs": ("table", ["pending > 0", "pending > 1", "pending > 4", "pending > 16", "after failure"], ["R=20"])},
    6: {"costs": ("table", ["pending > 0", "pending > 1", "pending > 4", "pending > 16", "after failure"], ["R=5", "R=20", "R=50"]),
        "best5": "str", "best20": "str", "best50": "str"},
}


_FORMAT_SPECS = {"week03": _FORMAT_WEEK03}
_FORMAT_SPECS.update({'week04': {1: {'reg_normal': 'number',
                'reg_rain': 'number',
                'cas_normal': 'number',
                'cas_rain': 'number'},
            2: {'reg_diff': 'number', 'cas_diff': 'number'},
            3: {'reg_model': 'model', 'reg_beta': 'number'},
            4: {'cas_model': 'model', 'cas_beta': 'number'},
            5: {'reg_low': 'number',
                'reg_high': 'number',
                'cas_low': 'number',
                'cas_high': 'number'},
            6: {'reg_naive_ratio': 'number',
                'cas_naive_ratio': 'number',
                'reg_adj_ratio': 'number',
                'cas_adj_ratio': 'number'}},
 'week05': {1: {'fp_half': 'int', 'fn_half': 'int'},
            2: {'cost_half_val': 'number', 'cost_none_val': 'number', 'cost_all_val': 'number'},
            3: {'threshold_default': 'number', 'cost_default': 'number'},
            4: {'selected': ('table', [0, 1, 2, 3], ['c_fp', 'c_fn', 'threshold', 'val_cost'])},
            5: {'evaluated': ('table',
                              [0, 1, 2, 3],
                              ['c_fp', 'c_fn', 'threshold', 'fp', 'fn', 'positive', 'test_cost'])},
            6: {'half_test_costs': ('list', 4), 'cost_ratios': ('list', 4)}},
 'week06': {1: {'group_counts': ('series', ['single', 'multiple']),
                'confirmed_rate': ('series', ['single', 'multiple'])},
            2: {'loop_counts': ('series', ['single', 'multiple'])},
            3: {'loop_cases': ('series', ['single', 'multiple']),
                'loops_per_case': ('series', ['single', 'multiple'])},
            4: {'phase_counts': ('series', ['single', 'multiple']),
                'phase_mean': ('series', ['single', 'multiple']),
                'phase_median': ('series', ['single', 'multiple'])},
            5: {'accepted_counts': ('series', ['single', 'multiple']),
                'incomplete_rate': ('series', ['single', 'multiple'])},
            6: {'mean_rounds': ('series', ['single', 'multiple']),
                'mean_days': ('series', ['single', 'multiple'])}},
 'week07': {1: {'lp': ('frame_columns',
                       ['case_id', 'timestamp', 'activity', 'lifecycle', 'resource']),
                'lead': ('series', None),
                'mean_lead': 'number'},
            2: {'w_counts': ('series', None), 'top_act': 'str'},
            3: {'n_arrivals': 'int',
                'c_agents': 'int',
                'mean_service': 'number',
                'a_load': 'number',
            ...
            4: {'required': 'int'},
            5: {'share': ('series', None)},
            6: {'queue_days': 'number', 'lead_no_queue': 'number', 'cut_ratio': 'number'}}})


def check_format(task, **values):
    """공개된 변수 자료형과 표 구조만 확인한다. 값의 정오를 판정하지 않는다."""
    if _WEEK not in _FORMAT_SPECS or task not in _FORMAT_SPECS[_WEEK]:
        raise ValueError("형식 확인을 지원하는 주차와 과제를 지정한다.")
    blank, bad = [], []
    for name, spec in _FORMAT_SPECS[_WEEK][task].items():
        value = values.get(name)
        if value is None:
            blank.append(name)
            continue
        if spec == "set":
            valid = isinstance(value, set) and all(isinstance(v, str) for v in value)
            detail = "디스크 번호 문자열의 집합"
        elif spec == "int":
            valid = isinstance(value, (int, np.integer)) and not isinstance(value, (bool, np.bool_))
            detail = "정수"
        elif spec == "number":
            valid = isinstance(value, (int, float, np.number)) and not isinstance(value, (bool, np.bool_))
            detail = "숫자"
        elif spec == "str":
            valid = isinstance(value, str)
            detail = "문자열"
        elif spec == "model":
            valid = hasattr(value, "params") and callable(getattr(value, "conf_int", None))
            detail = "적합한 회귀모형"
        elif spec[0] == "list":
            valid = (isinstance(value, list) and len(value) == spec[1]
                     and all(isinstance(v, (int, float, np.number)) and not isinstance(v, (bool, np.bool_)) for v in value))
            detail = "길이 %d의 숫자 목록" % spec[1]
        elif spec[0] == "series":
            valid = (isinstance(value, pd.Series) and value.index.is_unique
                     and pd.api.types.is_numeric_dtype(value.dtype)
                     and (spec[1] is None or set(value.index) == set(spec[1])))
            detail = "숫자 Series" + (" (인덱스: %s)" % ", ".join(spec[1]) if spec[1] else "")
        elif spec[0] == "frame_columns":
            valid = isinstance(value, pd.DataFrame) and set(spec[1]).issubset(value.columns)
            detail = "필수 열 %s를 가진 표" % ", ".join(spec[1])
        else:
            _, index, columns = spec
            valid = (isinstance(value, pd.DataFrame)
                     and value.index.is_unique and value.columns.is_unique
                     and set(value.index) == set(index)
                     and set(value.columns) == set(columns)
                     and all(pd.api.types.is_numeric_dtype(dtype) for dtype in value.dtypes))
            detail = "%d행 표, 행 이름 %s, 숫자 열 %s" % (len(index), ", ".join(map(str, index)), ", ".join(columns))
        if not valid:
            bad.append("%s: %s 필요" % (name, detail))
    if bad:
        _RESULTS[(_WEEK, task)] = "형식 수정 필요"
        print("형식 수정 필요: 과제 %s — %s" % (task, "; ".join(bad)))
        if blank:
            print("미작성: %s" % ", ".join(blank))
        return False
    if blank:
        _RESULTS[(_WEEK, task)] = "미작성"
        print("미작성: 과제 %s — %s" % (task, ", ".join(blank)))
        return False
    _RESULTS[(_WEEK, task)] = "형식 확인 완료"
    print("형식 확인 완료: 과제 %s — 계산값의 정답 여부는 제출 후 채점합니다." % task)
    return True
...
def check(task, **values):
    ...
    if _WEEK in _FORMAT_SPECS and not os.environ.get("BALAB_ANSWER_KEY"):
        raise RuntimeError("이 주차는 check_format으로 형식만 확인한다. 값 채점은 제출 후 수행한다.")
...
def summary():
    ...
    if _WEEK is None:
        ...
        return False
    if _WEEK in _FORMAT_SPECS and not os.environ.get("BALAB_ANSWER_KEY"):
        pending = [str(t) for t in _FORMAT_SPECS[_WEEK]
                   if _RESULTS.get((_WEEK, t)) != "형식 확인 완료"]
        if pending:
            print("형식 확인 미완료: 과제 %s — 미실행·미작성 또는 형식 수정 필요. 계산값은 채점하지 않았습니다." % ", ".join(pending))
            return False
        print("계산 과제 1~6 형식 확인 완료. 정답 판정은 아닙니다. 서술 답안도 작성해 제출하세요.")
        return True
